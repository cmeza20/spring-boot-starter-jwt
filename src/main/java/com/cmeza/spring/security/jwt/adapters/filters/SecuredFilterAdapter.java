...
public class SecuredFilterAdapter extends InitializingComponent implements SecuredFilterGateway {
    private JwtAuthenticationConverter jwtAuthenticationConverter;
    private JwtProviderUseCase jwtProviderUseCase;

    @Override
    public void doFilterInternalSecured(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain, RequestMatcher requestMatcher, JwtSecurityExceptionHandling jwtSecurityExceptionHandling, JwtUserDetailsServiceEvent jwtUserDetailsServiceEvent) throws ServletException, IOException {
        try {
            ...
            jwtUserDetailsServiceEvent.onSuccessfulAuthentication(httpServletRequest, httpServletResponse, authentication, PathType.SECURED);
            filterChain.doFilter(httpServletRequest, httpServletResponse);
        } catch (AuthenticationException e) {
            SecurityContextHolder.clearContext();
            jwtSecurityExceptionHandling.entryPointHandler(httpServletRequest, httpServletResponse, e);
            jwtUserDetailsServiceEvent.onUnsuccessfulAuthentication(httpServletRequest, httpServletResponse, e, PathType.SECURED);
        } catch (Exception e) {
            SecurityContextHolder.clearContext();
    ...
    protected void afterPropertiesSetComponent(ApplicationContext applicationContext, JwtSecurityConfigurer jwtSecurityConfigurer) {
        ...
        this.jwtAuthenticationConverter = new JwtAuthenticationConverter(jwtSecurityConfigurer, jwtSecurityConfigurer.getJwtProperties().getPaths().getSecured());
    }
}
